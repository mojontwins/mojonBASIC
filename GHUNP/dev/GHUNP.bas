...
# End Run-time Variables
 ...
 101 IF f=1 THEN LET a$="\i\j": LET b$="\k\l": LET c$="\m\n": LET d$="\o\p": LET f=0: GO SUB 50
 102 IF ATTR (w+2,a-3)=5 THEN LET z=3: LET x=-1: GO TO 105
 103 IF ATTR (w,a-2)=7 AND a>3 THEN IF ATTR (w+2,a-6)=5 THEN LET t=1: LET i=a-3: LET j=w: GO SUB 9500: LET t=13: LET i=a-6: GO SUB 9500: BEEP .01,0
 109 GO TO 30
 110 IF a=18 THEN LET n=n+1: LET a=0: RETURN
 111 IF f=0 THEN LET a$="\a\b": LET b$="\c\d": LET c$="\e\f": LET d$="\g\h": LET f=1: GO SUB 50
 112 IF ATTR (w+2,a+3)=5 THEN LET z=3: LET x=1: GO TO 115
 113 IF ATTR (w,a+4)=7 AND a<15 THEN IF ATTR (w+2,a+6)=5 THEN LET t=1: LET i=a+3: LET j=w: GO SUB 9500: LET t=13: LET i=a+6: GO SUB 9500: BEEP .01,0
 119 GO TO 30
 120 IF w=0 THEN LET n=n-6: LET w=15: RETURN
 121 IF ATTR (w-1,a)=5 THEN LET z=3: LET y=-1: GO TO 30
 122 IF ATTR (w-3,a+1)=7 AND w>3 THEN IF ATTR (w-4,a)=5 THEN LET t=1: LET i=a: LET j=w-3: GO SUB 9500: LET t=13: LET j=w-6: GO SUB 9500: BEEP .01,0
 129 GO TO 30
 130 IF w=15 THEN LET n=n+6: LET w=0: RETURN
 131 IF ATTR (w+5,a)=5 THEN LET z=3: LET y=1: GO TO 30
 132 IF ATTR (w+3,a+1)=7 AND w<12 THEN IF ATTR (w+8,a)=5 THEN LET t=1: LET i=a: LET j=w+3: GO SUB 9500: LET t=13: LET j=w+6: GO SUB 9500: BEEP .01,0
 139 GO TO 30
 149 REM QTILE?
 ...
 320 IF t=15 OR t=16 THEN GO TO 800
 ...
 599 REM CERROJOS
 600 IF a=6 THEN IF NOT f(1) THEN IF o(6) THEN LET f(1)=1: BEEP .3,30: BEEP .2,40
 610 IF a=12 THEN IF NOT f(2) THEN IF o(7) THEN LET f(2)=1: BEEP .3,30: BEEP .2,40
...
9805 LET w=0: LET a=0: LET i=0: LET x=0: LET y=0: LET j=0: LET z=0: LET d=0: DIM x$(27,9): DIM y$(27,9): DIM z$(27,9): LET a$="": LET b$="": LET c$="": LET d$="": LET k$="": LET n=0: LET on=0: DIM o(8): DIM f(8): LET t=0: LET r=0: LET m$="": LET n$="": LET h=0: LET s=0: LET q=0: LET m=0: LET b=0: LET c=0: BORDER NOT PI: PAPER NOT PI: INK 7: CLS : PRINT AT 11,11;"PREPARANDO": REM LAS VARIABLES MAS USADAS PRIMERO
...
9911 LET x$(11)=w$+"\@"+v$+"A"+v$+"B": LET y$(11)=u$+"_"+v$+"A"+u$+"_": LET z$(11)=t$+"_"+v$+"A"+u$+"_"
...
9927 LET x$(27)=x$(7): LET y$(27)=t$+"l"+u$+"F"+t$+"n": LET z$(27)=z$(7)
9990 GO TO 1000
